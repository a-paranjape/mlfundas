...
class Sequential(Module,MLUtilities,Utilities):
    ...
    #########################################
    def extract_hidden_features(self,layer=None):
        """ Extract specified layer of NN as collection of functions. 
            -- layer: int, index of hidden layer to extract. Default None (penultimate layer, same as self.extract_basis).
        """
        if self.verbose:
            self.print_this("... extracting hidden features",self.logfile)
        ...
        if layer is None:
            layer = params_setup['L']-1

        if (layer <= 0) | (layer >= params_setup['L']):
            raise Exception("Need 1 <= layer <= {0:d} or layer=None in Sequential.extract_hidden_features().".format(params_setup['L']-1))
            
        if self.verbose:
            self.print_this("... ... hidden layer {0:d} will be extracted".format(layer),self.logfile)

        for l in range(params_setup['L'],layer,-1):
            params_setup['L'] -= 1
...
class HyperOpt(Module,MLUtilities,Utilities):
    ...
    def __init__(self,setup_dict):
        ...
        self.loss_type = setup_dict.get('loss_type','square')
        self.neg_labels = setup_dict.get('neg_labels',True)

        self.standardize_X = setup_dict.get('standardize_X',True)
        self.standardize_Y = setup_dict.get('standardize_Y',True)
        self.max_epoch = setup_dict.get('max_epoch',1000000)
        self.check_after = setup_dict.get('check_after',300)
        self.decay_norm = setup_dict.get('decay_norm',2)
        self.test_type = setup_dict.get('test_type','perc')
        self.seed = setup_dict.get('seed',None)
        
        self.max_config = setup_dict.get('max_config',10)
        self.n_iter = setup_dict.get('n_iter',3)
        self.ensemble = setup_dict.get('ensemble',False)
        self.ensemble_size = setup_dict.get('ensemble_size',5) 
        self.parallel = setup_dict.get('parallel',False)
        self.nproc = setup_dict.get('nproc',4)

        self.verbose = setup_dict.get('verbose',True)
        self.logfile = setup_dict.get('logfile',None)

        self.layers = setup_dict.get('layers',{'min':1,'max':3})
        self.widths = setup_dict.get('widths',{'min':2,'max':2})
        self.fixed_width = setup_dict.get('fixed_width',True)
        self.lglrates = setup_dict.get('lglrates',{'min':-2.0,'max':-1.0})
...
class NetworkEnsembleObject(MLUtilities,Utilities):
    ...
    def load(self):
        ...
        for path in sorted(Path(self.ensemble_dir).glob("*.pkl")): 
            path_str = str(path)
...
#################################
# Wrapper to systematically develop NN using grid search
# DEPRECATED: Use HyperOpt instead
#################
class BuildNN(Module,MLUtilities,Utilities):
    ...
    def __init__(self,X=None,Y=None,train_frac=0.5,val_frac=0.2,n_iter=3,standardize_X=True,standardize_Y=True,
        ...
        print('THIS MODULE IS DEPRECATED. Use HyperOpt instead.')
        Utilities.__init__(self)
        self.X = X
        self.Y = Y
    ...
    def trainNN(self,max_epoch=1000000):
        ...
        if self.ensemble:
            sorter = np.argsort(tsvals)
            ensemble_size = np.max([2,tsvals.size // 20]) if tsvals.size > 2 else 1
            sorter = sorter[:ensemble_size] # select best 5% (if possible) of searched networks
            if self.verbose:
